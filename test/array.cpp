...
template<typename ArrayType> void array(const ArrayType& m)
{
  ...
  VERIFY_IS_APPROX(Eigen_tf::pow(m1,exponents), m1.square());
  VERIFY_IS_APPROX(m1.pow(exponents), m1.square());
  VERIFY_IS_APPROX(Eigen_tf::pow(2*m1,exponents), 4*m1.square());
  VERIFY_IS_APPROX((2*m1).pow(exponents), 4*m1.square());
  VERIFY_IS_APPROX(Eigen_tf::pow(m1,2*exponents), m1.square().square());
  VERIFY_IS_APPROX(m1.pow(2*exponents), m1.square().square());
  VERIFY_IS_APPROX(Eigen_tf::pow(m1(0,0), exponents), ArrayType::Constant(rows,cols,m1(0,0)*m1(0,0)));
...
template<typename ArrayType> void array_real(const ArrayType& m)
{
  ...
  VERIFY((m1.isNaN() == (Eigen_tf::isnan)(m1)).all());
  VERIFY((m1.isInf() == (Eigen_tf::isinf)(m1)).all());
  VERIFY((m1.isFinite() == (Eigen_tf::isfinite)(m1)).all());
  ...
  VERIFY((Eigen_tf::isnan)((m1*0.0)/0.0).all());
  VERIFY((Eigen_tf::isinf)(m4/0.0).all());
  VERIFY(((Eigen_tf::isfinite)(m1) && (!(Eigen_tf::isfinite)(m1*0.0/0.0)) && (!(Eigen_tf::isfinite)(m4/0.0))).all());
...
template<typename ArrayType> void array_complex(const ArrayType& m)
{
  ...
  VERIFY((Eigen_tf::isnan)(m1*zero/zero).all());
#if EIGEN_COMP_MSVC
  // msvc complex division is not robust
  VERIFY((Eigen_tf::isinf)(m4/RealScalar(0)).all());
#else
#if EIGEN_COMP_CLANG
  // clang's complex division is notoriously broken too
  if((numext::isinf)(m4(0,0)/RealScalar(0))) {
#endif
    VERIFY((Eigen_tf::isinf)(m4/zero).all());
#if EIGEN_COMP_CLANG
  }
  else
  {
    VERIFY((Eigen_tf::isinf)(m4.real()/zero.real()).all());
  }
#endif
#endif // MSVC

  VERIFY(((Eigen_tf::isfinite)(m1) && (!(Eigen_tf::isfinite)(m1*zero/zero)) && (!(Eigen_tf::isfinite)(m1/zero))).all());
