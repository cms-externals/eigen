...
//   VERIFY_RAISES_ASSERT(mf+md); // does not even compile
  ...
  // check scalar powers
  VERIFY_MIX_SCALAR( pow(vcf.array(), sf),        Eigen_tf::pow(vcf.array(), complex<float>(sf)) );
  VERIFY_MIX_SCALAR( vcf.array().pow(sf) ,        Eigen_tf::pow(vcf.array(), complex<float>(sf)) );
  VERIFY_MIX_SCALAR( pow(sd, vcd.array()),        Eigen_tf::pow(complex<double>(sd), vcd.array()) );
  VERIFY_MIX_SCALAR( Eigen_tf::pow(vf.array(), scf), Eigen_tf::pow(vf.template cast<complex<float> >().array(), scf) );
  VERIFY_MIX_SCALAR( vf.array().pow(scf) ,        Eigen_tf::pow(vf.template cast<complex<float> >().array(), scf) );
  VERIFY_MIX_SCALAR( Eigen_tf::pow(scd, vd.array()), Eigen_tf::pow(scd, vd.template cast<complex<double> >().array()) );
