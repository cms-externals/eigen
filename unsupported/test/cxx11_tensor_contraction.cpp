...
static void test_evals()
{
  ...
  mat4.setZero();
  Eigen_tf::array<DimPair, 1> dims3 = {{DimPair(0, 0)}};
  ...
  mat5.setZero();
  Eigen_tf::array<DimPair, 1> dims4 = {{DimPair(1, 1)}};
  ...
  mat6.setZero();
  Eigen_tf::array<DimPair, 1> dims6 = {{DimPair(1, 0)}};
...
static void test_scalar()
{
  ...
  Eigen_tf::array<DimPair, 1> dims = {{DimPair(0, 0)}};
...
static void test_multidims()
{
  ...
  mat3.setZero();
  Eigen_tf::array<DimPair, 2> dims = {{DimPair(1, 2), DimPair(2, 3)}};
  ...
  mat6.setZero();
  Eigen_tf::array<DimPair, 2> dims2({{DimPair(0, 1), DimPair(1, 0)}});
...
static void test_holes() {
  ...
  Eigen_tf::array<DimPair, 2> dims = {{DimPair(0, 0), DimPair(3, 4)}};
...
static void test_contraction_of_contraction()
{
  ...
  Eigen_tf::Map<Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout>>
      m1(t1.data(), 2, 2), m2(t2.data(), 2, 2), m3(t3.data(), 2, 2),
      m4(t4.data(), 2, 2);
  Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout>
...
static void test_out_of_order_contraction()
{
  ...
  Eigen_tf::array<DimPair, 2> dims = {{DimPair(2, 0), DimPair(0, 2)}};
  mat3 = mat1.contract(mat2, dims);
  ...
  Eigen_tf::array<DimPair, 2> dims2 = {{DimPair(0, 2), DimPair(2, 0)}};
...
static void test_consistency()
{
  ...
  Eigen_tf::array<DimPair, 2> dims1 = {{DimPair(0, 4), DimPair(1, 0)}};
  Eigen_tf::array<DimPair, 2> dims2 = {{DimPair(4, 0), DimPair(0, 1)}};
...
static void test_large_contraction()
{
  ...
  typedef Map<Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout>> MapXf;
  MapXf m_left(t_left.data(), 1500, 248);
  MapXf m_right(t_right.data(), 248, 1400);
  Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout> m_result(1500, 1400);

  // this contraction should be equivalent to a single matrix multiplication
  Eigen_tf::array<DimPair, 2> dims = {{DimPair(2, 0), DimPair(3, 1)}};
...
static void test_matrix_vector()
{
  ...
  Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout> m_result(30, 1);
...
static void test_tensor_vector()
{
  ...
  Eigen_tf::array<DimensionPair, 1> dim_pair01{{{0, 1}}};
  ...
  Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout> m_result = m_left.transpose() * m_right.transpose();
...
static void test_small_blocking_factors()
{
  ...
  Eigen_tf::setCpuCacheSizes(896, 1920, 2944);
  ...
  Map<Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout>> m_left(t_left.data(), 150, 93);
  Map<Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout>> m_right(t_right.data(), 93, 140);
  Eigen_tf::Matrix<float, Dynamic, Dynamic, DataLayout> m_result = m_left * m_right;
...
static void test_tensor_product()
{
  ...
  Tensor<float, 4, DataLayout> result = mat1.contract(mat2, Eigen_tf::array<DimPair, 0>{{}});
