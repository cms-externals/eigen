...
struct CPUContext {
  CPUContext(const Eigen_tf::Tensor<float, 3>& in1, Eigen_tf::Tensor<float, 3>& in2, Eigen_tf::Tensor<float, 3>& out) : in1_(in1), in2_(in2), out_(out), kernel_1d_(2), kernel_2d_(2,2), kernel_3d_(2,2,2) {
    kernel_1d_(0) = 3.14f;
    kernel_1d_(1) = 2.7f;
  ...
  const Eigen_tf::DefaultDevice& device() const { return cpu_device_; }

  const Eigen_tf::Tensor<float, 3>& in1() const { return in1_; }
  const Eigen_tf::Tensor<float, 3>& in2() const { return in2_; }
  Eigen_tf::Tensor<float, 3>& out() { return out_; }
  const Eigen_tf::Tensor<float, 1>& kernel1d() const { return kernel_1d_; }
  const Eigen_tf::Tensor<float, 2>& kernel2d() const { return kernel_2d_; }
  const Eigen_tf::Tensor<float, 3>& kernel3d() const { return kernel_3d_; }

 private:
  const Eigen_tf::Tensor<float, 3>& in1_;
  const Eigen_tf::Tensor<float, 3>& in2_;
  Eigen_tf::Tensor<float, 3>& out_;

  Eigen_tf::Tensor<float, 1> kernel_1d_;
  Eigen_tf::Tensor<float, 2> kernel_2d_;
  Eigen_tf::Tensor<float, 3> kernel_3d_;

  Eigen_tf::DefaultDevice cpu_device_;
};


// Context for evaluation on GPU
struct GPUContext {
  GPUContext(const Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in1, Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in2, Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& out) : in1_(in1), in2_(in2), out_(out), gpu_device_(&stream_) {
  ...
  const Eigen_tf::GpuDevice& device() const { return gpu_device_; }

  const Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in1() const { return in1_; }
  const Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in2() const { return in2_; }
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& out() { return out_; }
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 1> > kernel1d() const { return Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 1> >(kernel_1d_, 2); }
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 2> > kernel2d() const { return Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 2> >(kernel_2d_, 2, 2); }
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> > kernel3d() const { return Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >(kernel_3d_, 2, 2, 2); }

 private:
  const Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in1_;
  const Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& in2_;
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> >& out_;

  float* kernel_1d_;
  float* kernel_2d_;
  float* kernel_3d_;

  Eigen_tf::CudaStreamDevice stream_;
  Eigen_tf::GpuDevice gpu_device_;
};
...
{
  Eigen_tf::array<std::pair<int, int>, 2> dims;
  dims[0] = std::make_pair(1, 1);
  dims[1] = std::make_pair(2, 2);

  Eigen_tf::array<int, 2> shape(40, 50*70);

  Eigen_tf::DSizes<int, 2> indices(0,0);
  Eigen_tf::DSizes<int, 2> sizes(40,40);
...
{
  Eigen_tf::DSizes<int, 3> indices(0,0,0);
  Eigen_tf::DSizes<int, 3> sizes(40,49,70);

  Eigen_tf::array<int, 1> dims(1);
...
{
  Eigen_tf::DSizes<int, 3> indices(0,0,0);
  Eigen_tf::DSizes<int, 3> sizes(40,49,69);

  Eigen_tf::array<int, 2> dims(1,2);
...
{
  Eigen_tf::DSizes<int, 3> indices(0,0,0);
  Eigen_tf::DSizes<int, 3> sizes(39,49,69);

  Eigen_tf::array<int, 3> dims(0,1,2);
  context->out().slice(indices, sizes).device(context->device()) = context->in1().convolve(context->kernel3d(), dims);
}


void test_cpu() {
  Eigen_tf::Tensor<float, 3> in1(40,50,70);
  Eigen_tf::Tensor<float, 3> in2(40,50,70);
  Eigen_tf::Tensor<float, 3> out(40,50,70);
...
void test_gpu() {
  ...
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> > gpu_in1(d_in1, 40,50,70);
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> > gpu_in2(d_in2, 40,50,70);
  Eigen_tf::TensorMap<Eigen_tf::Tensor<float, 3> > gpu_out(d_out, 40,50,70);
