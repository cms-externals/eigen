...
void test_multithread_elementwise()
{
  ...
  Eigen_tf::ThreadPool tp(internal::random<int>(3, 11));
  Eigen_tf::ThreadPoolDevice thread_pool_device(&tp, internal::random<int>(3, 11));
...
void test_multithread_contraction()
{
  ...
  Eigen_tf::ThreadPool tp(4);
  Eigen_tf::ThreadPoolDevice thread_pool_device(&tp, 4);
 ...
 for (ptrdiff_t i = 0; i < t_result.size(); i++) {
    ...
    if (Eigen_tf::internal::isApprox(t_result(i), m_result(i), 1e-4f)) {
      continue;
    }
...
void test_contraction_corner_cases()
{
  ...
  Eigen_tf::ThreadPool tp(12);
  Eigen_tf::ThreadPoolDevice thread_pool_device(&tp, 12);
...
void test_multithread_contraction_agrees_with_singlethread() {
  ...
  typedef Tensor<float, 1>::DimensionPair DimPair;
  Eigen_tf::array<DimPair, 1> dims({{DimPair(1, 2)}});

  Eigen_tf::ThreadPool tp(internal::random<int>(2, 11));
  Eigen_tf::ThreadPoolDevice thread_pool_device(&tp, internal::random<int>(2, 11));
...
void test_multithreaded_reductions() {
  ...
  Eigen_tf::ThreadPoolDevice thread_pool_device(&thread_pool, num_threads);
...
void test_memcpy() {
  ...
  for (int i = 0; i < 5; ++i) {
    const int num_threads = internal::random<int>(3, 11);
    Eigen_tf::ThreadPool tp(num_threads);
    Eigen_tf::ThreadPoolDevice thread_pool_device(&tp, num_threads);
...
{
  Eigen_tf::ThreadPool tp(2);
  Eigen_tf::ThreadPoolDevice device(&tp, 2);
  Tensor<float, 1> t(1 << 20);
  t.device(device) = t.random<Eigen_tf::internal::NormalRandomGenerator<float>>();
}
...
void test_multithread_shuffle()
{
  ...
  Eigen_tf::ThreadPoolDevice device(&threads, num_threads);
