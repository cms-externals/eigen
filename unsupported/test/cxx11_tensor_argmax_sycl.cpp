...
static void test_sycl_simple_argmax(const Eigen_tf::SyclDevice &sycl_device){

  Tensor<DataType, 3, Layout, DenseIndex> in(Eigen_tf::array<DenseIndex, 3>{{2,2,2}});
  ...
  Eigen_tf::TensorMap<Eigen_tf::Tensor<DataType, 3, Layout, DenseIndex> > gpu_in(d_in, Eigen_tf::array<DenseIndex, 3>{{2,2,2}});
  Eigen_tf::TensorMap<Eigen_tf::Tensor<DenseIndex, 0, Layout, DenseIndex> > gpu_out_max(d_out_max);
  Eigen_tf::TensorMap<Eigen_tf::Tensor<DenseIndex, 0, Layout, DenseIndex> > gpu_out_min(d_out_min);
...
static void test_sycl_argmax_dim(const Eigen_tf::SyclDevice &sycl_device)
{
  ...
  for (DenseIndex dim = 0; dim < 4; ++dim) {
    ...
    Eigen_tf::TensorMap<Eigen_tf::Tensor<DataType, 4, DataLayout, DenseIndex> > gpu_in(d_in, Eigen_tf::array<DenseIndex, 4>{{sizeDim0,sizeDim1,sizeDim2,sizeDim3}});
    Eigen_tf::TensorMap<Eigen_tf::Tensor<DenseIndex, 3, DataLayout, DenseIndex> > gpu_out(d_out, out_shape);
...
template <typename DataType, int DataLayout, typename DenseIndex>
static void test_sycl_argmin_dim(const Eigen_tf::SyclDevice &sycl_device)
{
  DenseIndex sizeDim0=9;
  DenseIndex sizeDim1=3;
  DenseIndex sizeDim2=5;
