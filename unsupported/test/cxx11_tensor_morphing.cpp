...
static void test_reshape_as_lvalue()
{
  ...
  tensor5d.reshape(dim).device(Eigen_tf::DefaultDevice()) = tensor;
...
static void test_simple_slice()
{
  ...
  Eigen_tf::DSizes<ptrdiff_t, 5> indices(1,2,3,4,5);
  Eigen_tf::DSizes<ptrdiff_t, 5> sizes(1,1,1,1,1);
  slice1 = tensor.slice(indices, sizes);
  VERIFY_IS_EQUAL(slice1(0,0,0,0,0), tensor(1,2,3,4,5));

  Tensor<float, 5, DataLayout> slice2(1,1,2,2,3);
  Eigen_tf::DSizes<ptrdiff_t, 5> indices2(1,1,3,4,5);
  Eigen_tf::DSizes<ptrdiff_t, 5> sizes2(1,1,2,2,3);
...
static void test_slice_in_expr() {
  ...
  Eigen_tf::DSizes<ptrdiff_t, 2> indices1(1,2);
  Eigen_tf::DSizes<ptrdiff_t, 2> sizes1(3,3);
  Eigen_tf::DSizes<ptrdiff_t, 2> indices2(0,2);
  Eigen_tf::DSizes<ptrdiff_t, 2> sizes2(3,1);
...
static void test_slice_as_lvalue()
{
  ...
  Eigen_tf::DSizes<ptrdiff_t, 3> sizes12(2,2,7);
  Eigen_tf::DSizes<ptrdiff_t, 3> first_slice(0,0,0);
  result.slice(first_slice, sizes12) = tensor1;
  Eigen_tf::DSizes<ptrdiff_t, 3> second_slice(2,0,0);
  result.slice(second_slice, sizes12).device(Eigen_tf::DefaultDevice()) = tensor2;

  Eigen_tf::DSizes<ptrdiff_t, 3> sizes3(4,3,5);
  Eigen_tf::DSizes<ptrdiff_t, 3> third_slice(0,2,0);
  result.slice(third_slice, sizes3) = tensor3;

  Eigen_tf::DSizes<ptrdiff_t, 3> sizes4(4,3,2);
  Eigen_tf::DSizes<ptrdiff_t, 3> fourth_slice(0,2,5);
  ...
  Eigen_tf::DSizes<ptrdiff_t, 3> sizes5(4,5,7);
  Eigen_tf::DSizes<ptrdiff_t, 3> fifth_slice(0,0,0);
...
static void test_slice_raw_data()
{
  ...
  Eigen_tf::DSizes<ptrdiff_t, 4> offsets(1,2,3,4);
  Eigen_tf::DSizes<ptrdiff_t, 4> extents(1,1,1,1);
  ...
  if (DataLayout == ColMajor) {
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(2,1,1,1);
    ...
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(1,1,1,2);
    auto slice2 = SliceEvaluator(tensor.slice(offsets, extents), DefaultDevice());
    VERIFY_IS_EQUAL(slice2.dimensions().TotalSize(), 2);
    VERIFY_IS_EQUAL(slice2.data()[0], tensor(1,2,3,4));
  ...
  extents = Eigen_tf::DSizes<ptrdiff_t, 4>(1,2,1,1);
  ...
  if (DataLayout == ColMajor) {
    offsets = Eigen_tf::DSizes<ptrdiff_t, 4>(0,2,3,4);
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(3,2,1,1);
    ...
    offsets = Eigen_tf::DSizes<ptrdiff_t, 4>(1,2,3,0);
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(1,1,2,11);
    auto slice4 = SliceEvaluator(tensor.slice(offsets, extents), DefaultDevice());
  ...
  if (DataLayout == ColMajor) {
    offsets = Eigen_tf::DSizes<ptrdiff_t, 4>(0,0,0,4);
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(3,5,7,2);
    ...
    offsets = Eigen_tf::DSizes<ptrdiff_t, 4>(1,0,0,0);
    extents = Eigen_tf::DSizes<ptrdiff_t, 4>(2,5,7,11);
    auto slice5 = SliceEvaluator(tensor.slice(offsets, extents), DefaultDevice());
  ...
  offsets = Eigen_tf::DSizes<ptrdiff_t, 4>(0,0,0,0);
  extents = Eigen_tf::DSizes<ptrdiff_t, 4>(3,5,7,11);
...
static void test_strided_slice()
{
  ...
  typedef Eigen_tf::DSizes<Eigen_tf::DenseIndex, 5> Index5;
  typedef Tensor<float, 2, DataLayout> Tensor2f;
  typedef Eigen_tf::DSizes<Eigen_tf::DenseIndex, 2> Index2;
  Tensor<float, 5, DataLayout> tensor(2,3,5,7,11);
  ...
  if(true) {
    ...
    Eigen_tf::DSizes<Eigen_tf::DenseIndex, 5> indicesStart(1, 2, 3, 4, 5);
    Eigen_tf::DSizes<Eigen_tf::DenseIndex, 5> indicesStop(2, 3, 4, 5, 6);
    Eigen_tf::DSizes<Eigen_tf::DenseIndex, 5> strides(1, 1, 1, 1, 1);
...
{
  Eigen_tf::Tensor<float, 2, DataLayout> matrix(7, 11);
  matrix.setRandom();

  const DSizes<ptrdiff_t, 3> newDims(1, 1, 11);
  Eigen_tf::Tensor<float, 3, DataLayout> tensor =
