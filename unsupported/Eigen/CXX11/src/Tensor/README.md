...
```Eigen_tf::array<Eigen_tf::Index>```.  The array can be constructed automatically
...
have to know the size of the result.
    ...
    Eigen_tf::Tensor<float, 2> c(30, 40);
    c.device(...) = a + b;
...
#### Evaluating with a Thread Pool
    ...
    Eigen_tf::ThreadPoolDevice my_device(4 /* number of threads to use */);

    // Now just use the device when evaluating expressions.
    Eigen_tf::Tensor<float, 2> c(30, 50);
...
The actual type of the dimensions() result is <Tensor-Type>::Dimensions.
    ...
    Eigen_tf::Tensor<float, 2> a(3, 4);
    const Eigen_tf::Tensor<float, 2>::Dimensions& d = a.dimensions();
...
is not initialized.
    ...
    Eigen_tf::TensorFixedSize<float, Sizes<3, 4>> a;
...
to accomodate of the coefficients of the tensor.
    ...
    Eigen_tf::TensorMap<Tensor<float, 2>> a(data, 3, 4);
    cout << "NumRows: " << a.dimension(0) << " NumCols: " << a.dimension(1) << endl;
    => NumRows: 3 NumCols: 4
...
has a copy constructor and an ```operator=()```:
    ...
    Eigen_tf::Tensor<string, 2> a(2, 3);
    a.setConstant("yolo");
...
See ```struct UniformRandomGenerator``` in TensorFunctors.h for an example.
    ...
    struct MyRandomGenerator {
      ...
      // be ignored.
      Scalar operator()(Eigen_tf::DenseIndex element_location,
                        Eigen_tf::DenseIndex /*unused*/ = 0) const {
        return <randomly generated value of type T>;
      }

      // Same as above but generates several numbers at a time.
      typename internal::packet_traits<Scalar>::type packetOp(
          Eigen_tf::DenseIndex packet_location, Eigen_tf::DenseIndex /*unused*/ = 0) const {
...
tensor, or multiply every element of a tensor by a scalar.
    ...
    Eigen_tf::Tensor<float, 2> a(2, 3);
    a.setConstant(1.0f);
    Eigen_tf::Tensor<float, 2> b = a + a.constant(2.0f);
    Eigen_tf::Tensor<float, 2> c = b * b.constant(0.2f);
    cout << "a" << endl << a << endl << endl;
...
as for ```setRandom()```.
    ...
    Eigen_tf::Tensor<float, 2> a(2, 3);
    a.setConstant(1.0f);
    Eigen_tf::Tensor<float, 2> b = a + a.random();
    cout << "a" << endl << a << endl << endl;
    cout << "b" << endl << b << endl << endl;
    =>
    a
...
cubic roots of an int Tensor:
    ...
    Eigen_tf::Tensor<double, 2> b = a.cast<double>().pow(1.0 / 3.0);
    cout << "a" << endl << a << endl << endl;
    cout << "b" << endl << b << endl << endl;
    =>
    a
...
Example: Reduction along one dimension.
    ...
    Eigen_tf::array<int, 1> dims({1 /* dimension to reduce */});
    // ...using the "maximum" operator.
    // The result is a tensor with one dimension.  The size of
    // that dimension is the same as the first (non-reduced) dimension of a.
    Eigen_tf::Tensor<int, 1> b = a.maximum(dims);
    cout << "a" << endl << a << endl << endl;
    cout << "b" << endl << b << endl << endl;
    =>
    a
...
Example: Reduction along two dimensions.
    ...
    Eigen_tf::Tensor<float, 3, Eigen_tf::ColMajor> a(2, 3, 4);
    ...
    Eigen_tf::Tensor<float, 1, Eigen_tf::ColMajor> b =
        a.maximum(Eigen_tf::array<int, 2>({0, 1}));
    cout << "b" << endl << b << endl << endl;
    =>
    b
    20
...
scalar, represented as a zero-dimension tensor.
    ...
    Eigen_tf::Tensor<float, 3> a(2, 3, 4);
    a.setValues({{{0.0f, 1.0f, 2.0f, 3.0f},
                  {7.0f, 6.0f, 5.0f, 4.0f},
                  {8.0f, 9.0f, 10.0f, 11.0f}},
                 {{12.0f, 13.0f, 14.0f, 15.0f},
    ...
    Eigen_tf::Tensor<float, 0> b = a.sum();
    cout << "b" << endl << b << endl << endl;
    =>
    b
    276
...
dd a comment to this line
    ...
    Eigen_tf::Tensor<int, 2> b = a.cumsum(1);
...
for the last dimension).
    ...
    Eigen_tf::array<ptrdiff_t, 2> dims({1, 2});  // Specify second and third dimension for convolution.
...
to one dimension:
    ...
    Eigen_tf::Tensor<float, 2, Eigen_tf::ColMajor> a(2, 3);
    a.setValues({{0.0f, 100.0f, 200.0f}, {300.0f, 400.0f, 500.0f}});
    Eigen_tf::array<Eigen_tf::DenseIndex, 1> one_dim({3 * 2});
    Eigen_tf::Tensor<float, 1, Eigen_tf::ColMajor> b = a.reshape(one_dim);
    cout << "b" << endl << b << endl;
    =>
    b
      0
...
This is what happens when the 2D Tensor is RowMajor:
    ...
    Eigen_tf::Tensor<float, 2, Eigen_tf::RowMajor> a(2, 3);
    a.setValues({{0.0f, 100.0f, 200.0f}, {300.0f, 400.0f, 500.0f}});
    Eigen_tf::array<Eigen_tf::DenseIndex, 1> one_dim({3 * 2});
    Eigen_tf::Tensor<float, 1, Eigen_tf::RowMajor> b = a.reshape(one_dim);
    cout << "b" << endl << b << endl;
    =>
    b
      0
...
The previous example can be rewritten as follow:
    ...
    Eigen_tf::Tensor<float, 2, Eigen_tf::ColMajor> a(2, 3);
    a.setValues({{0.0f, 100.0f, 200.0f}, {300.0f, 400.0f, 500.0f}});
    Eigen_tf::array<Eigen_tf::DenseIndex, 2> two_dim({2, 3});
    Eigen_tf::Tensor<float, 1, Eigen_tf::ColMajor> b(6);
    b.reshape(two_dim) = a;
    cout << "b" << endl << b << endl;
    =>
    b
...
For example this is what happens when you ```stride()``` a 2D tensor:
    ...
    Eigen_tf::Tensor<int, 2> a(4, 3);
    a.setValues({{0, 100, 200}, {300, 400, 500}, {600, 700, 800}, {900, 1000, 1100}});
    Eigen_tf::array<Eigen_tf::DenseIndex, 2> strides({3, 2});
    Eigen_tf::Tensor<int, 2> b = a.stride(strides);
    cout << "b" << endl << b << endl;
    =>
    b
       0   200
...
the input tensor.
    ...
    Eigen_tf::array<int, 2> offsets = {1, 0};
    Eigen_tf::array<int, 2> extents = {2, 2};
    Eigen_tf::Tensor<int, 1> slice = a.slice(offsets, extents);
    cout << "a" << endl << a << endl;
    =>
    a
       0   100   200
...
matrix.
    ...
    a.setValues({{0, 100, 200}, {300, 400, 500},
                 {600, 700, 800}, {900, 1000, 1100}});
    Eigen_tf::Tensor<int, 1> row_3 = a.chip(2, 0);
    Eigen_tf::Tensor<int, 1> col_2 = a.chip(1, 1);
    cout << "a" << endl << a << endl;
    =>
    a
       0   100   200
...
lvalue. For example:
    ...
    Eigen_tf::Tensor<int, 1> a(3);
    a.setValues({{100, 200, 300}});
    Eigen_tf::Tensor<int, 2> b(2, 3);
    b.setZero();
    b.chip(0, 0) = a;
    cout << "a" << endl << a << endl;
    =>
...
of a 2D tensor:
    ...
    a.setValues({{0, 100, 200}, {300, 400, 500},
                {600, 700, 800}, {900, 1000, 1100}});
    Eigen_tf::array<bool, 2> reverse({true, false});
    Eigen_tf::Tensor<int, 2> b = a.reverse(reverse);
...
made in each of the dimensions.
    ...
    Eigen_tf::Tensor<int, 2> a(2, 3);
    a.setValues({{0, 100, 200}, {300, 400, 500}});
    Eigen_tf::array<int, 2> bcast({3, 2});
    Eigen_tf::Tensor<int, 2> b = a.broadcast(bcast);
    cout << "a" << endl << a << endl << "b" << endl << b << endl;
    =>
    a
       0   100   200
...
Returns a view of the input tensor in which the input is padded with zeros.
    ...
    Eigen_tf::Tensor<int, 2> a(2, 3);
    a.setValues({{0, 100, 200}, {300, 400, 500}});
    Eigen_tf::array<pair<int, int>, 2> paddings;
    paddings[0] = make_pair(0, 1);
    paddings[1] = make_pair(2, 3);
    Eigen_tf::Tensor<int, 2> b = a.pad(paddings);
    cout << "a" << endl << a << endl << "b" << endl << b << endl;
    =>
    a
       0   100   200
...
For example, given the following input tensor:
  ...
  Eigen_tf::Tensor<float, 2, DataLayout> tensor(3,4);
...
Six 2x2 patches can be extracted and indexed using the following code:
  ...
  Eigen_tf::Tensor<float, 3, DataLayout> patch;
  Eigen_tf::array<ptrdiff_t, 2> patch_dims;
  patch_dims[0] = 2;
  patch_dims[1] = 2;
...
type T.
    ...
    Eigen_tf::Tensor<float, 2> a(2, 3);
    Eigen_tf::Tensor<int, 2> b = a.cast<int>();
