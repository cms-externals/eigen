...
/** \ingroup SparseCore_Module
  ...
  *          whereas it is now (starting from %Eigen 3.3) deprecated and always defined as Eigen_tf::Index.
  ...
  : public SparseCompressedBase<SparseMatrix<_Scalar, _Options, _StorageIndex> >
{
  ...
  public:
    ...
    /** Preallocates \a reserveSize[\c j] non zeros for each column (resp. row) \c j.
      ...
      * Typical choices include std::vector<int>, Eigen_tf::VectorXi, Eigen_tf::VectorXi::Constant, etc.
      */
  ...
  public:
    ...
    inline void setIdentity()
    {
      ...
      Eigen_tf::Map<IndexVector>(this->m_data.indexPtr(), rows()).setLinSpaced(0, StorageIndex(rows()-1));
      Eigen_tf::Map<ScalarVector>(this->m_data.valuePtr(), rows()).setOnes();
      Eigen_tf::Map<IndexVector>(this->m_outerIndex, rows()+1).setLinSpaced(0, StorageIndex(rows()));
      std::free(m_innerNonZeros);
      m_innerNonZeros = 0;
    }
...
/** Fill the matrix \c *this with the list of \em triplets defined by the iterator range \a begin - \a end.
  ...
  * \endcode
  * See for instance the Eigen_tf::Triplet template class.
  *
...
EIGEN_DONT_INLINE SparseMatrix<Scalar,_Options,_StorageIndex>& SparseMatrix<Scalar,_Options,_StorageIndex>::operator=(const SparseMatrixBase<OtherDerived>& other)
{
  ...
  if (needToTranspose)
  {
    ...
    Eigen_tf::Map<IndexVector> (dest.m_outerIndex,dest.outerSize()).setZero();
